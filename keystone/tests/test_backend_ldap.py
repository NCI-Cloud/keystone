...
# is submitted - see Bugs 1092187, 1101287, 1101276, 1101289
    ...
    def test_user_id_attribute_in_create(self):
        conf = self.get_config(CONF.identity.default_domain_id)
        conf.ldap.user_id_attribute = 'email'
        self.reload_backends(CONF.identity.default_domain_id)

        user = {'name': u'fÃ¤kÃ©1',
                'id': uuid.uuid4().hex,
                'password': u'fÃ¤kÃ©pass1',
                'domain_id': CONF.identity.default_domain_id}
        # NOTE(gyee): the user_id parameter for create_user() is not
        # used by ldap backend.
        user = self.identity_api.create_user(user['id'], user)
